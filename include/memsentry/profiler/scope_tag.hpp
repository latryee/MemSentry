...
class ScopedTag {
public:
    explicit ScopedTag(const char* tag) noexcept : prev_(g_active_scope_tag) { g_active_scope_tag = tag; }

    ~ScopedTag() noexcept { g_active_scope_tag = prev_; }

    ScopedTag(const ScopedTag&) = delete;
    ScopedTag& operator=(const ScopedTag&) = delete;

    [[nodiscard]] static const char* current() noexcept { return g_active_scope_tag ? g_active_scope_tag : "General"; }
