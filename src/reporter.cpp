...
void ConsoleReporter::print_summary(std::ostream& os, const MemoryStatsSnapshot& stats, size_t leak_count,
                                    size_t leak_bytes) {
    os << "\n\033[1;36m================================================================================\033[0m\n";
    ...
    if (leak_count == 0) {
        ...
        os << "\033[1;31m [STATUS] MEMORY LEAKS DETECTED: " << leak_count << " block(s) unreleased ("
...
void ConsoleReporter::print_leaks(std::ostream& os, const std::vector<AllocationRecord>& leaks) {
    ...
    for (size_t i = 0; i < leaks.size(); ++i) {
        ...
        os << "\033[1;33m[Leak #" << (i + 1) << "]\033[0m ID: " << rec.allocation_id << " | Size: \033[1;37m"
...
void ConsoleReporter::print_diff(std::ostream& os, const profiler::SnapshotDiff& diff) {
    os << "\n\033[1;36m================================================================================\033[0m\n";
    os << "\033[1;37m                     SNAPSHOT DIFF: [" << diff.before_label << "] -> [" << diff.after_label
       << "]\033[0m\n";
    os << "\033[1;36m================================================================================\033[0m\n";
    os << " Net Memory Delta    : " << (diff.net_bytes_delta >= 0 ? "+" : "") << diff.net_bytes_delta << " bytes\n";
    os << " Net Alloc Delta     : " << (diff.net_allocations_delta >= 0 ? "+" : "") << diff.net_allocations_delta
       << " blocks\n";
...
void ConsoleReporter::print_corruption_alert(std::ostream& os, const void* ptr, CorruptionType type) {
    ...
    os << " Reason  : ";
    switch (type) {
    case CorruptionType::HEADER_CORRUPTED:
        os << "Buffer underrun / Block header magic overwritten\n";
        break;
    case CorruptionType::FOOTER_CORRUPTED:
        os << "Buffer overrun / Red-zone canary overwritten\n";
        break;
...
bool JsonReporter::write_file(const std::string& filepath, const MemoryStatsSnapshot& stats,
...
std::string HtmlReporter::generate(const MemoryStatsSnapshot& stats, const std::vector<AllocationRecord>& records) {
    ...
    for (const auto& b : hist.buckets()) {
        if (b.count > max_bucket_count)
            max_bucket_count = b.count;
    }
    for (const auto& b : hist.buckets()) {
        ...
        ss << "    <div class=\"histogram-track\"><div class=\"histogram-fill\" style=\"width:" << pct
           << "%\"></div></div>\n";
    ...
    for (size_t i = 0; i < records.size(); ++i) {
        ...
        if (rec.frame_count > 0) {
            ...
            for (size_t f = 0; f < frames.size(); ++f) {
                const auto& frame = frames[f];
                ss << "      <div class=\"stack-line\">#" << f << " <span class=\"stack-sym\">" << frame.symbol_name
                   << "</span>";
                if (!frame.file_name.empty()) {
...
bool HtmlReporter::write_file(const std::string& filepath, const MemoryStatsSnapshot& stats,
                              const std::vector<AllocationRecord>& records) {
    std::ofstream out(filepath);
    if (!out.is_open())
        return false;
